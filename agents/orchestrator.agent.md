...
When delegated work needs GitHub-hosted reference content outside the current repo, route agents to the Pandora's Box MCP server instead of raw GitHub browsing or generic web fetches.

Priority:

1. use local workspace files when the target repo is already checked out
2. otherwise use Pandora's Box `get_github_file_content`, `list_github_repos`, and `search_github_repo_patterns`
3. reserve `web` for non-repository pages and `context7` for library/framework docs

Apply this especially to content in `JOHNEPPILLAR/the-synth-eng-skills` and `JOHNEPPILLAR/the-synth-eng-code-ref`.
...
- `SoftwareEngineer`: small implementation or terminal work; hidden internal subagent
- `PrincipalEngineer`: complex implementation or terminal work; hidden internal subagent
...
- discovery/scouting strategy -> `../skills/research-discovery/SKILL.md`
- independent review output contract -> `../skills/review-core/SKILL.md`
- post-implementation review routing -> `../skills/review-orchestration/SKILL.md`
- multi-model consensus consolidation -> `../skills/multi-model-review/SKILL.md`
...
3. **Implementation** -> `SoftwareEngineer` or `PrincipalEngineer`
...
Before any task that requires file edits, delegate a **Tool Preflight** to the intended executor (`SoftwareEngineer`, `PrincipalEngineer`, `Designer`, or `Debugger`).
...
Before terminal-heavy work that depends on command execution, delegate a **Terminal Preflight** to the intended executor (`SoftwareEngineer`, `PrincipalEngineer`, `Debugger`, or `Verifier`).
...
1. use `SoftwareEngineer` first for simpler work; escalate to `PrincipalEngineer` as needed
...
2. delegate durable memory writes to `SoftwareEngineer` with `ALLOW_MEMORY_UPDATE=true`
...
3. `Orchestrator` decides when to create/merge/cleanup worktrees, but MUST delegate the actual terminal commands for these actions to `SoftwareEngineer` or `PrincipalEngineer`.
...
4. main `Orchestrator` keeps sole decision ownership of worktree create/merge/cleanup (delegating the commands to `SoftwareEngineer`)
