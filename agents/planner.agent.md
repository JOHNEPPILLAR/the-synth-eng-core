...
When planning depends on GitHub-hosted skills, examples, or reference code outside the current repo, fetch that repository content through the Pandora's Box MCP server instead of browsing raw GitHub pages.

Priority:

1. use local workspace files when the referenced repo is available locally
2. otherwise use Pandora's Box `get_github_file_content`, `list_github_repos`, and `search_github_repo_patterns`
3. use `context7` only for external libraries/frameworks, not repo-owned skill files or reference examples

This applies especially to `JOHNEPPILLAR/the-synth-eng-skills` and `JOHNEPPILLAR/the-synth-eng-code-ref`.
...
- API contract and evolution work -> fetch `skills/api-design/SKILL.md` from `JOHNEPPILLAR/the-synth-eng-skills` through Pandora's Box
